    ...
    while ( taille_msg != 0 ) {
        ...
        {   
            ...
            if (code_retour == PAQUET_RECU){
                de_reseau(&reponse);
                // printf("verif_control = %d, dans_fenetre = %d, cdt = %d\n",verifier_controle(reponse), dans_fenetre(borne_inf,reponse.num_seq,TAILLE_FEN), verifier_controle(reponse) && dans_fenetre(borne_inf,reponse.num_seq,TAILLE_FEN));
                // printf("verif_control = %d, dans_fenetre = %d, cdt = %d\n",verifier_controle(reponse), dans_fenetre(borne_inf,reponse.num_seq,TAILLE_FEN), verifier_controle(reponse) && dans_fenetre(borne_inf,reponse.num_seq,TAILLE_FEN));
